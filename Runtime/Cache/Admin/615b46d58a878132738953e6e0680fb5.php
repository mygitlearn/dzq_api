...
<div class="row-fluid header">
    ...
    <div class="span10 pull-right">
        <input type="text" class="span5 search" placeholder="请输入用户名或账号" value="<?php echo session('searchCondition')?>"/>
        <a href="<?php echo U('addUser');?>" class="btn-flat success pull-right"><span>&#43;</span>新增管理员</a>
    </div>
</div>
...
<div class="row-fluid table">
    <table class="table table-hover">
        <thead>
            <tr>
                <th class="span4 sortable">用户</th>
                <th class="span3 sortable"><span class="line"></span>联系方式</th>
                <th class="span3 sortable"><span class="line"></span>电子邮箱</th>
                <th class="span2 sortable"><span class="line"></span>最近登录</th>
        ...
        <tbody>
            <?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <td>
                        <img src="<?php echo ($vo["avatar"]); ?>" class="img-circle avatar hidden-phone" style="width: 45px;height: 45px;"/>
                        <!--<a href="#" class="name"><?php echo ($vo["name"]); ?>（<?php echo ($vo["account"]); ?>）</a>-->
                        <span style="line-height: 3"> <?php echo ($vo["name"]); ?>（<?php echo ($vo["account"]); ?>）</span>
                    </td>
                    <td><?php echo ($vo["phone"]); ?></td>
                    <td><?php echo ($vo["email"]); ?></td>
                    <td><?php echo ($vo["logo_time"]); ?></td>
                    <td class="align-right">
                        <a href="<?php echo U('User/delManager?id='.$vo['token']);?>" style="color: red">删除</a>
                    </td>
...
<script src="/dzq_api/Public/static/mstp/js/layout.js"></script>
    ...
    <script type="text/javascript" src="/dzq_api/Public/Admin/js/admin/manager.js"></script>
